...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.label_embedding = nn.Embedding(100, 64 * 64)
        
        self.conv1 = nn.Conv2d(4, 64, kernel_size=4, stride=2, padding=1, bias=False)
    ...
    def forward(self, x, labels):
        ...
        label_input = label_input.reshape(-1, 1, 64, 64)
        
        con = torch.cat([x, label_input], dim=1)
        
        out = self.relu1(self.norm1(self.conv1(con)))
...
class Generator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        # label_input = label_input.reshape(-1, 1, 32, 32)
        input = torch.cat([x, label_input], 1)
        input = input.reshape(x.shape[0], 200, 1, 1)
